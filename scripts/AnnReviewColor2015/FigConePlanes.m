...
try
    ...
    calFactorsAll = importdata(fullfile('FigConePlanes_Data','BearFruitGrayB','calibration.txt'));
    calFactors = calFactorsAll(:,2);
    
    % Read the image
    imRawSizePixels = 2020;
    for i = 1:length(wls)
        imageNameIn = fullfile('FigConePlanes_Data','BearFruitGrayB',num2str(wls(i)));
