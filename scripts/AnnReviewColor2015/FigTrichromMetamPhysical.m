...
load sur_mccBabel
theRawSpd = SplineSpd(S_mccBabel,...
...
% Scale to some reasonable luminance
load T_xyz1931
T_xyz = SplineCmf(S_xyz1931,683*T_xyz1931,S);
T_lum = T_xyz(2,:);
rawLum = T_lum*theRawSpd;
theSpdEnergy = 100*theRawSpd/rawLum;
theSpdQuanta = EnergyToQuanta(S,theSpdEnergy);
theSpd = RadianceToRetIrradiance(theSpdQuanta,S,pi*(1.5^2),17);
theSpdCones = T_conesQETrichrom*theSpd*(pi*(0.5^2));
...
theMetamerCones = T_conesQETrichrom*theMetamer*(pi*(0.5^2));
...
ylabel('Isomerization Rate (iso/cone-sec)','FontName',figParams.fontName,'FontSize',figParams.labelFontSize);
...
xlabel('Wavelength (nm)','FontName',figParams.fontName,'FontSize',figParams.labelFontSize);
%ylim([figParams.yLimLow figParams.yLimHigh]);
ylabel('Radiance (quanta/sec-um2)','FontName',figParams.fontName,'FontSize',figParams.labelFontSize);
%set(gca,'YTick',figParams.yTicks);
%set(gca,'YTickLabel',figParams.yTickLabels);
axis('square');
%set(gca,'XMinorTick','on');
